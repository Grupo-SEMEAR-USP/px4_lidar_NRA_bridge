...
PX4_Lidar_Bridge::PX4_Lidar_Bridge(const ros::NodeHandle& nh)
    : nh_(nh) {
  ...
  worker_ = std::thread(&PX4_Lidar_Bridge::publishSystemStatus, this);


};

PX4_Lidar_Bridge::~PX4_Lidar_Bridge() { }


void PX4_Lidar_Bridge::odomCallback(const nav_msgs::Odometry& msg) {
...
void PX4_Lidar_Bridge::publishSystemStatus(){
  ...
  while(ros::ok()){
    ...
    if(flag_first_pose_received == true) { // only send heartbeat if we receive pose estimates at all
      ...
      if( (ros::Time::now()-last_callback_time) > ros::Duration(0.5) ){
        ROS_WARN_STREAM("Stopped receiving data from lidar");
        system_status_ = MAV_STATE::MAV_STATE_FLIGHT_TERMINATION;
      }
